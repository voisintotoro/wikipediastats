...
public class URLDecoderSafer {
  ...
  private static final String[] SPECIALS_CHARS_REPLACEMENT = new String[] {
      "%20", "%21", "%22", "%23", "%24", "%26", "%27", "%28", "%29", "%2A",
      "%2B", "%2C", "%2D", "%2E", "%2F", "%3A", "%3B", "%3C", "%3D", "%3E",
      "%3F", "%40", "%5B", "%5C", "%5D", "%5E", "%5F", "%60", "%7B", "%7C",
      "%7D", "%7E" };
    ...
    if (url != null) {
      ...
      try {
        ...
        if (StringUtils.containsAny(tmpUrl, SPECIALS_CHARS_AS_CHAR)) {
          tmpUrl = StringUtils.replaceEach(tmpUrl, SPECIALS_CHARS,
              SPECIALS_CHARS_REPLACEMENT);
        }
